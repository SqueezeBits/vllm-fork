...
def sample_dataset_requests(
...
) -> List[Tuple[str, int, int, None, Optional[str]]]:
    ...
    for i, req in enumerate(dataset):
        ...
        try:
            ...
            lora_adapter = req.get(lora_key, None)
        ...
        input_requests.append((prompt, len(prompt_ids), output_len, None, lora_adapter))
    return input_requests
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            if output_len is None:
                ...
                # NOTE(SQZB): use outputs[i].num_generated_token for accurate benchmarks.
                # output_len = len(
                #     tokenizer(outputs[i].generated_text,
                #               add_special_tokens=False).input_ids)
                output_len = outputs[i].num_generated_token
    ...
    metrics = BenchmarkMetrics(
        ...
        max_ttft_ms=np.max(ttfts or 0.0) * 1000,
...
async def benchmark(
    ...
    if len(input_requests) > 0:
        ...
        test_prompt, test_prompt_len, test_output_len, test_mm_content, test_lora_adapter = (
            input_requests[0])
    ...
    async for request in get_request(input_requests, request_rate, burstiness):
        prompt, prompt_len, output_len, mm_content, lora_adapter = request
...
def main(args: argparse.Namespace):
    ...
    if args.dataset is not None:
        ...
        dataset_info = json.loads(args.dataset_path)
        input_requests = sample_dataset_requests(
            dataset_path=dataset_info["name"],
            num_requests=args.num_prompts,
            input_len=args.input_len,
            output_len=args.output_len,
            tokenizer=tokenizer,
            split=dataset_info["split"],
            prompt_key=dataset_info["prompt_key"],
            input_key=dataset_info["input_key"],
            lora_key=dataset_info.get("lora_key", None)
        )
    ...
    if args.save_result:
        ...
        result_json["repetition_penalty"] = args.repetition_penalty
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Apply top_p sampling.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=-1,
        help="Apply top_k sampling.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0,
        help="Apply temperature sampling.",
    )
    parser.add_argument(
        "--repetition-penalty",
        type=float,
        default=1.0,
        help="Apply repetition_penalty sampling.",
    )
