...
def bgmv(
    ...
    punica_kernels.dispatch_bgmv(y, x, w_t_all, indices, layer_idx, scale)
    ...
    if is_hpu():
        for i in range(x.size(0)):
            if int(indices[i]) >= 0:
                y[i] += (
                    x[i].unsqueeze(0)
                    @ wa_t_all[int(indices[i]), layer_idx, :, :].transpose(-1, -2)
                    @ wb_t_all[int(indices[i]), layer_idx, :, :].transpose(-1, -2)
                    * scale
                  ).squeeze(0)
        # Extract the specific weights for this instance
        # for i in range(x.size(0)):
        #     wa_t = wa_t_all[int(indices[i]), layer_idx, :, :]  # Shape [R, H1]
        #     wb_t = wb_t_all[int(indices[i]), layer_idx, :, :]  # Shape [H2, R]
            
        #     # Buffer computation: buffer[i] = x[i] @ wa_t^T
        #     # x[i].unsqueeze(0) has shape [1, H1]
        #     # wa_t.transpose(-1, -2) has shape [H1, R]
        #     # Resulting in buffer[i] having shape [R]
        #     buffer[i] = torch.matmul(x[i].unsqueeze(0), wa_t.transpose(-1, -2)).squeeze(0)
        # for i in range(buffer.size(0)):
        #     # Final computation: y[i] += buffer[i] @ wb_t * scale
        #     # buffer[i].unsqueeze(0) has shape [1, R]
        #     # wb_t.transpose(-1, -2) has shape [R, H2]
        #     # Resulting in shape [H2]
        #     y[i] += torch.matmul(buffer[i].unsqueeze(0), wb_t.transpose(-1, -2)).squeeze(0) * scale
    else:
        try:
            import vllm._punica_C as punica_kernels
        except ImportError as e:
        ...
        if buffer is None:
            ...
            buffer = torch.zeros((x.size(0), r),
                                dtype=torch.float32,
                                device=x.device)
        punica_kernels.dispatch_bgmv(buffer, x, wa_t_all, indices, layer_idx, 1.0)
        punica_kernels.dispatch_bgmv(y, buffer, wb_t_all, indices, layer_idx, scale)
